...
document.addEventListener('DOMContentLoaded', function() {
  ...
  headings.forEach((heading, index) => {
    ...
    tocLink.addEventListener('click', function(e) {
      e.preventDefault();
      console.log('点击了 TOC 链接:', heading.tagName, heading.id); // 调试日志

      // 滚动到对应标题 - 使用 getElementById 而不是 querySelector
      // 因为 querySelector 对特殊字符（如中文、冒号）处理不好
      const targetHeading = document.getElementById(heading.id);
      if (targetHeading) {
        targetHeading.scrollIntoView({
          behavior: 'smooth'
        });
        console.log('滚动到标题:', heading.id); // 调试日志
      } else {
        console.error('未找到目标标题:', heading.id); // 错误日志
      }
    ...
    console.log('添加了 TOC 项:', heading.tagName, heading.textContent); // 调试日志
  });

  console.log('TOC 生成完成，共添加了', headings.length, '个标题'); // 调试日志
  ...
  function updateActiveTocItem(activeId) {
    ...
    // 为当前标题对应的目录项添加活动状态
    if (currentHeading) {
      // 使用 getAttribute 而不是 querySelector 来避免特殊字符问题
      let activeLink = null;
